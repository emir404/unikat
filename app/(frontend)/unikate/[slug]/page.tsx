...
import type { Piece } from "@/lib/defaults";
...
export async function generateStaticParams() {
  ...
  return c.pieces.map((piece) => ({ slug: piece.id }));
}
...
export async function generateMetadata({
  ...
  const c = await getSiteContent();
  const piece = c.pieces.find((p) => p.id === slug);
  if (!piece) return {};
  return {
...
export default async function UnikatPage({
  ...
  const index = pieces.findIndex((p) => p.id === slug);
  if (index === -1) notFound();
  const piece: Piece = pieces[index];
  const atelier = c.atelier;
  ...
  return (
    <div className="flex min-h-svh flex-col bg-background">
          ...
          <div className="flex flex-col lg:pt-2">
            ...
            <Reveal delay={0.2} className="mt-10">
              <dl>
                ...
                <SpecRow
                  label="Anfertigung"
                  value={`Von Hand im Atelier, ${atelier.street}, ${atelier.city}`}
                />
              </dl>
            </Reveal>
      ...
      <Footer atelier={atelier} footer={c.footer} curtain={false} />
    </div>
  );
}

...
import type { Atelier, BesuchContent, HourEntry } from "@/lib/defaults";
        ...
        <div className="mt-20 grid gap-14 lg:mt-28 lg:grid-cols-[minmax(0,1fr)_minmax(0,42%)] lg:gap-20">
          <div>
            ...
            <div className="mt-8 [perspective:900px]">
              {hours.map((entry, i) => {
                const isToday = openState?.day === entry.weekday;
                const isClosed = entry.openStart === null;
                return (
                  <motion.div
                    key={entry.weekday}
                    initial={
      ...
      <div className="relative mt-16 lg:mt-24">
        <Reveal y={60} amount={0.15}>
          <div className="relative aspect-[4/3] w-full overflow-clip sm:aspect-[16/9] lg:aspect-[21/9]">
            <iframe
              ...
              title={`${atelier.name} auf Google Maps – ${atelier.street}, ${atelier.zip} ${atelier.city}`}
        ...
        >
          ...
          >
            {atelier.street}
            <br />
            {atelier.zip} {atelier.city} · {atelier.district}
          </a>
          <a
            href={atelier.mapsUrl}
            target="_blank"

...
import { AccentHeading, EASE, useReducedMotionSafe } from "./Reveal";
import type { Atelier, HeroContent, HeroSlide, NavLink } from "@/lib/defaults";
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIndices(({ active: current }) => ({
        active: (current + 1) % slides.length,
        prev: current,
      }));
    }, SLIDE_MS);
    return () => clearTimeout(timer);
        ...
        <div className="mt-7 flex w-full flex-col gap-10 lg:flex-row lg:items-end lg:justify-between lg:gap-20">
          ...
          >
            <div className="flex w-full flex-col gap-2.5">
              ...
              <span className="relative block overflow-hidden">
                <AnimatePresence mode="popLayout" initial={false}>
                  ...
                  >
                    {[slide.material, slide.surface].filter(Boolean).join(" · ")}
                  </motion.p>
                </AnimatePresence>
              </span>
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              >
                {content.ctaSecondary} — {atelier.phoneDisplay}
                <span
                  aria-hidden
                  className="inline-block transition-transform duration-300 group-hover:translate-x-1.5"
                >
